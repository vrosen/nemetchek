...
<div class="col-md-6">
      <form method="POST" action="{{ route('item.update', $item->id) }}" accept-charset="UTF-8" class="col-12">
    @csrf
    @method('PUT')
    <div class="form-group">
        ...
        name="name" type="text" id="name" value="{{ $item->name }}" autocomplete="off">

        <label for="name" class="control-label mt-2 required">{{ __('Description') }}</label>
        <textarea class="form-control mt-1" name="description">{{ $item->description }}</textarea>
    </div>
